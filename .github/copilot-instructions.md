...
- **Cycles render "all black" in mm-scale scenes** — `scene.unit_settings.scale_length = 0.001` means 1 blender unit = 1mm. Area lights at `energy=5` are effectively invisible at this scale. **Working values (confirmed): key light 150000W at `(150,-60,200)`, fill 40000W at `(-80,120,60)`, light size=150. World background `(0.15,0.15,0.15)` at strength=1.2.** World ambient above strength≈1.5 causes a "cloud" effect — all shadows wash out and displacement becomes invisible. Always add a World background node; without it, unlit areas are pure black.
