...
_SKIN_PNG   = (
    pathlib.Path(r"C:\QIDISrc\QIDIStudio\install_dir\resources\assets"
...
def _find_skin() -> pathlib.Path:
    ...
    for p in (_REPO_ROOT / "resources" / "assets").rglob("*.png"):
        return p
    raise FileNotFoundError("No skin PNG found — set _SKIN_PNG manually in ai_debug_pipeline.py")
...
# ── Blender invocation ────────────────────────────────────────────────────────
def _run_blender(blender: str, model_path: str, skin_path: str,
                 snapshots_dir: pathlib.Path, log_path: pathlib.Path,
                 projection: str = "auto") -> tuple[int, str]:
...
def _validate_snapshots(snapshots_dir: pathlib.Path, expected_class: str) -> dict:
    ...
    if ok and not mismatch_flag:
        ...
        remark = (f"PASS-with-warning — class {actual_class} correct but Shape DNA flagged "
                  f"TOPOLOGY MISMATCH. Check eigenvalue ratio.")
    else:
        remark = (f"FAIL — classified as {actual_class} but expected {expected_class}. "
...
def _remediation_hint(result: dict, case: dict) -> str:
    ...
    lines.append(f"  Measured: sharp_fraction={sf}, z_ratio={zr}, "
                 f"curvature_std={cs}, euler_characteristic={chi}")

    if actual == "FLAT_SHELL" and expected != "FLAT_SHELL":
        lines.append("  → z_ratio too low — part classified as FLAT_SHELL because z_ratio < threshold.")
        lines.append("    Try lowering FLAT_SHELL z_ratio upper bound in _classify_mesh_topology().")

    elif actual == "REVOLUTION" and expected != "REVOLUTION":
        lines.append("  → z_ratio ≥ REVOLUTION threshold and sharp_fraction is low.")
        lines.append("    If model is actually PRISMATIC, raise PRISMATIC sharp_fraction threshold.")

    elif actual == "PRISMATIC" and expected == "FLAT_SHELL":
        lines.append("  → sharp_fraction is above PRISMATIC threshold for a flat part.")
        lines.append("    Lower PRISMATIC sharp_fraction threshold or raise FLAT_SHELL guard.")

    elif actual == "ORGANIC" and expected != "ORGANIC":
        lines.append("  → curvature_std is above ORGANIC threshold.")
        lines.append("    If this is not organic, raise the ORGANIC curvature_std entry threshold.")

    if result["mismatch_flag"]:
        lines.append("  → Shape DNA TOPOLOGY MISMATCH: λ1/λ2 ratio contradicts classifier.")
...
def run(cases: list[dict] | None = None, render_heatmap: bool = False) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="qidi_debug_stl_") as tmp_stl_dir:
        ...
        for i, case in enumerate(cases, 1):
            ...
            if not pathlib.Path(model_3mf).exists():
                ...
                print(f"         {msg}\n")
                all_results.append({
                    "name": name, "ok": None, "remark": msg, "model": model_3mf,
                })
                continue
            ...
            rc, blender_stderr = _run_blender(
                blender_exe, model_path_for_blender, str(skin_path),
                snap_dir, log_path, projection=proj,
            )
            print(f"rc={rc}")
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="Autonomous AI debug harness for apply_texture_bpy.py")
    ap.add_argument("--case", action="append", default=None,
                    help="Run only this case name (repeat for multiple). Default: all cases.")
    ap.add_argument("--render-heatmap", action="store_true",
