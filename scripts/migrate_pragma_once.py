...
def process_file(path: pathlib.Path, dry_run: bool = False, backup: bool = False) -> bool:
    """
    ...
    if backup:
        path.with_suffix(path.suffix + '.guard_backup').write_text(text, encoding='utf-8')
...
def main() -> None:
    ...
    parser.add_argument('--vendor-dirs', nargs='*',
                        default=['earcut', 'mcut', 'clipper', 'qhull', 'libvgcode',
                                 'expat', 'zlib', 'miniz', 'boost'],
    ...
    print()
    if args.dry_run:
        print(f'DRY RUN complete: {changed} files would be migrated, '
              f'{unchanged} already clean, {skipped} vendored (skipped).')
    else:
        print(f'Migration complete: {changed} files migrated, '
              f'{unchanged} already clean, {skipped} vendored (skipped).')
