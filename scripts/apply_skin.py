...
_LOG_PATH = pathlib.Path(os.environ.get("TEMP", str(pathlib.Path.home()))) / "apply_skin_log.txt"
...
class _Tee:
    ...
    def __init__(self, stream, log_fh):
        ...
        self._log    = log_fh
    def write(self, msg):
        try: self._stream.write(msg); self._stream.flush()
...
for _pkg in ("numpy", "trimesh", "PIL"):
    try: __import__(_pkg)
    except ImportError: MISSING.append(_pkg)

if MISSING:
    print(f"ERROR: Missing packages: {', '.join(MISSING)}")
    print(f"  Run:  pip install {' '.join(MISSING)}")
    _log_fh.close(); sys.exit(1)
...
def _load_raw(asset_path: str, size_px: int = 512,
              invert: bool = True, gamma: float = 0.7) -> "np.ndarray":
    """
    ...
    if ext == ".svg":
        try:
            ...
            png = cairosvg.svg2png(url=asset_path,
    ...
    if span > 1e-4:
        arr = (arr - lo) / span
    print(f"  Heightmap raw: {arr.shape}  min={lo:.3f}  max={hi:.3f}  (stretched to 0..1)")
...
def _sample_tiled(hmap: "np.ndarray", u_mm: "np.ndarray", v_mm: "np.ndarray",
                  tile_mm: float) -> "np.ndarray":
    """
...
def _project_cylindrical(verts: "np.ndarray", norms: "np.ndarray",
                          bb_min: "np.ndarray", bb_max: "np.ndarray",
                          hmap: "np.ndarray", tile_mm: float,
                          long_axis: int) -> "np.ndarray":
    """
    ...
    print(f"  Projection: cylindrical (long_axis={'XYZ'[long_axis]}, "
          f"R≈{mean_radius:.1f} mm, {n_tiles} tiles/rev, "
          f"u_tile={tile_mm_u:.2f} mm)")
    return _sample_tiled(hmap, u_mm, v_mm, tile_mm_u)


def _project_triplanar(verts: "np.ndarray", norms: "np.ndarray",
                        bb_min: "np.ndarray", bb_max: "np.ndarray",
                        hmap: "np.ndarray", tile_mm: float) -> "np.ndarray":
    """
    ...
    return np.where(dominant == 0, samp_xy,
           np.where(dominant == 1, samp_xz,
                                   samp_yz))


def _pick_projector(bb_min: "np.ndarray", bb_max: "np.ndarray",
    ...
    if med_dim > 0 and (long_dim / med_dim) >= 2.5:
        ...
        if verts is not None:
            try:
                ...
                if hull.geom_type == "Polygon" and hull.length > 0:
                    ...
                    verdict = "cylindrical" if iq >= 0.55 else "triplanar (flat cross-section)"
        ...
        if iq >= 0.55:
            ...
            return lambda v, n, mn, mx, h, t: _project_cylindrical(v, n, mn, mx, h, t, long_axis)
...
def _displace(mesh: "trimesh.Trimesh", hmap: "np.ndarray",
              tile_mm: float,
              bb_min=None, bb_max=None,
              max_edge_hint: float = 2.5) -> "trimesh.Trimesh":
    """
    ...
    if max_edge_hint > 0:
        ...
        print(f"  Displacement clamped to ≤ {max_safe:.3f} mm (40% of target edge {max_edge_hint:.3f} mm)")
...
def main() -> int:
    ...
    p.add_argument("--log",        default=None,
                   help="Override log file path (QIDIStudio passes this)")
    args = p.parse_args()

    # ── Validate paths ────────────────────────────────────────────────────────
    if not os.path.exists(args.model_path):
        print(f"ERROR: model file not found:\n  {args.model_path}"); return 1
    if not os.path.exists(args.skin_path):
        print(f"ERROR: skin asset not found:\n  {args.skin_path}");  return 1

    print()
    print(f"Model : {args.model_path}")
    print(f"Skin  : {args.skin_path}")
    print(f"Tile  : {args.tile_size} mm   Relief: {args.relief} mm   Max-edge: {args.max_edge} mm")
    print(f"Invert: {args.invert}   Gamma: {args.gamma}")
    print()
    ...
    try:
        ...
        print(f"ERROR reading 3MF: {e}"); traceback.print_exc(); return 1

    def _ns(tag): return f"{{{ns3}}}{tag}"
    ...
    if not mesh_objects:
        print("ERROR: no mesh objects found in 3MF"); return 1
    ...
    print(f"  {len(mesh_objects)} mesh object(s) | "
    ...
    print(f"\n─── Building heightmap ───")
    try:
        single = _load_raw(args.skin_path, invert=args.invert, gamma=args.gamma) * args.relief
    except Exception as e:
        print(f"ERROR building heightmap: {e}"); traceback.print_exc(); return 1

    # ── Per-object: subdivide → displace (global BB) → update XML in-place ────
    print(f"\n─── Processing {len(mesh_objects)} object(s) ───")
    total_out_verts = 0

    for idx, (verts_el, tris_el, vtag, ttag, loc_verts, loc_faces) in enumerate(mesh_objects):
        ...
        if args.max_edge > 0:
            try:
                new_v, new_f = trimesh.remesh.subdivide_to_size(
                    local_mesh.vertices, local_mesh.faces,
                    max_edge=args.max_edge, max_iter=6)
        ...
        displaced_local = _displace(local_mesh, single, args.tile_size,
                                     bb_min=global_bb_min, bb_max=global_bb_max,
        ...
        print(f"  obj[{idx}]: {v_in} src → {len(displaced_local.vertices):,} displaced verts")

        # Rewrite <vertices> in-place
        for v in list(verts_el): verts_el.remove(v)
        for vx, vy, vz in displaced_local.vertices:
            el = _ET.SubElement(verts_el, vtag)
            el.set("x", f"{vx:.6f}"); el.set("y", f"{vy:.6f}"); el.set("z", f"{vz:.6f}")

        # Rewrite <triangles> in-place
        for t in list(tris_el): tris_el.remove(t)
        for i0, i1, i2 in displaced_local.faces:
            el = _ET.SubElement(tris_el, ttag)
            el.set("v1", str(int(i0))); el.set("v2", str(int(i1))); el.set("v3", str(int(i2)))

    # ── Serialise every updated .model tree back to bytes ─────────────────────
    for fname, (tree, root_el) in model_trees.items():
        zip_bytes[fname] = (
            b'<?xml version="1.0" encoding="UTF-8"?>\n' +
            _ET.tostring(root_el, encoding="unicode", xml_declaration=False).encode("utf-8")
        )
    ...
    try:
        ...
        print(f"ERROR saving 3MF: {e}"); traceback.print_exc(); return 1

    kb = out_p.stat().st_size // 1024
    print(f"  Written: {out_p}  ({kb:,} KB)  |  {total_out_verts:,} total displaced verts")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n=== Unhandled exception ==="); traceback.print_exc(); rc = 1
    finally:
        print(f"\n=== Exit code: {rc} ===")
        try: _log_fh.close()
        except Exception: pass
