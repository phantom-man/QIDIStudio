...
SLICER_PARAMS: dict[str, dict[str, Any]] = {
    ...
    "perimeter_speed": {
        ...
        "max": 300.0,
        "unit": "mm/s",
        "desc": "Perimeter (wall) print speed",
    },
    "infill_speed": {
        "type": float,
        "min": 20.0,
    ...
    "perimeters": {
        ...
        "max": 20,
        "unit": "walls",
        "desc": "Number of perimeter walls",
    },
    "bottom_solid_layers": {
        "type": int,
        "min": 1,
    ...
    "first_layer_temperature": {
        ...
        "max": 330.0,
        "unit": "°C",
        "desc": "First layer nozzle temperature (°C)",
    },
    "bed_temperature": {
        "type": float,
        "min": 0.0,
    ...
    "support_material_threshold": {
        ...
        "max": 90.0,
        "unit": "°",
        "desc": "Support overhang threshold (degrees)",
    },
    # Retraction
    "retract_length": {
        "type": float,
...
def _call_gemini(prompt: str) -> list[dict]:
    ...
    if not api_key:
        print(
            "[nl_slicer] GOOGLE_API_KEY not set; using rule-based fallback",
            file=sys.stderr,
        )
    ...
    try:
        ...
        try:
            ...
            print(
                "[nl_slicer] No Gemini SDK installed; using rule-based fallback",
                file=sys.stderr,
            )
            return _rule_based_fallback(prompt)
...
def _rule_based_fallback(prompt: str) -> list[dict]:
    ...
    if any(
        w in p
        for w in ["watertight", "sealed", "no gap", "tight", "fluid", "enclosure"]
    ):
        changes += [
    ...
    if any(
        w in p
        for w in ["draft", "test", "prototype", "quick check", "throwaway", "fit check"]
    ):
        changes += [
    ...
    if any(w in p for w in ["honeycomb"]):
        changes += [
            {"key": "fill_pattern", "value": "honeycomb", "reason": "fill_pattern"}
        ]
    if any(w in p for w in ["gyroid"]):
        changes += [
            {"key": "fill_pattern", "value": "gyroid", "reason": "fill_pattern"}
        ]
    ...
    if any(
        w in p
        for w in ["don't mind waiting", "waiting", "maximise detail", "maximize detail"]
    ):
        changes += [
...
SMOKE_PROMPTS = [
    ...
    # ─── Original 10 ───────────────────────────────────────────────────────────
    (
        "Make it stronger and print faster",
        ["fill_density", "perimeters", "perimeter_speed"],
    ),
    ...
    (
        "Print it as fast as you can, quality doesn't matter",
    ...
    (
        "I'm printing flexible TPU, reduce the speed",
    ...
    (
        "Quick throwaway test, don't care about quality",
        ["layer_height", "fill_density"],
    ),
    (
        "Strong functional part, increase fill to at least 50%",
        ["fill_density", "perimeters"],
    ),
    (
        "Speed run, print in under 2 hours",
        ["infill_speed", "perimeter_speed", "layer_height"],
    ),
    ("Grid infill for a flexible mat", ["fill_pattern"]),
    ("Maximum perimeters for a hollow vase", ["perimeters"]),
    (
        "Slow down and maximise detail, I don't mind waiting",
        ["layer_height", "perimeter_speed"],
    ),
]
...
def run_smoke_tests() -> bool:
    ...
    for i, (prompt, expected_keys) in enumerate(SMOKE_PROMPTS, 1):
        ...
        if ok:
            ...
            failed_tests.append(
                f"  [{i}] '{prompt}' — expected {expected_keys}, got {sorted(changed_keys)}"
            )

        print(f"  [{status}] {i:2d}. {prompt[:60]}")
        for ch in changes:
            print(
                f"       {ch.get('key','?')}={ch.get('value')} ({ch.get('reason','')})"
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="NL -> QIDIStudio slicer settings (Phase 6.1)"
    )
    parser.add_argument(
        "--prompt", type=str, default="", help="Natural language print request"
    )
    parser.add_argument(
        "--profile", type=str, default="", help="Path to profile JSON (read+update)"
    )
    parser.add_argument(
        "--output", type=str, default="", help="Output JSON path (default: stdout)"
    )
    parser.add_argument("--verbose", action="store_true", help="Verbose output")
    parser.add_argument(
        "--smoke-test", action="store_true", help="Run 10 standard smoke test prompts"
    )
