...
MAX_STRETCH_DEGENERATE        = 5.0    # max_stretch > 5 = collapsed UV island
...
def _analyse_uv_stretch(stage_data: dict, report: CriticReport) -> None:
    ...
    if not stretch or "error" in stretch:
        err_msg = stretch.get("error", "no uv_stretch in snapshot") if stretch else "no uv_stretch in snapshot"
        # OBJECT-coords mode won't have UV stretch — not an error
        proj = stage_data.get("projection", "object")
        if proj == "object":
            report.add(DiagnosticIssue(
                severity="INFO", stage="post_displace", signal="metric",
            ...
            report.add(DiagnosticIssue(
                severity="WARNING", stage="post_displace", signal="metric",
    ...
    # ── Check 3: Degenerate UV island (collapsed face) ────────────────────
...
def format_report(report: CriticReport) -> str:
    ...
    if not report.issues:
        lines.append("  No issues detected. Pipeline is operating within normal parameters.")
    else:
