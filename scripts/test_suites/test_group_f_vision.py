...
VENV_PY = REPO_ROOT / ".venv" / "Scripts" / "python.exe"  # pyvista lives here
...
def _run_py(script: str, timeout: int = 120) -> tuple[bool, str]:
    ...
    return _run_subprocess(MEMORY_PY, script, timeout)


def _run_venv_py(script: str, timeout: int = 90) -> tuple[bool, str]:
    """Run a Python snippet using .venv (has pyvista, VTK) with reliable kill on timeout."""
    py = VENV_PY if VENV_PY.exists() else MEMORY_PY
    return _run_subprocess(py, script, timeout)


def _run_subprocess(python_exe: Path, script: str, timeout: int) -> tuple[bool, str]:
    """Core subprocess runner — Popen + communicate so kill works on Windows."""
    import signal
...
"""
    ok, output = _run_py(script, timeout=120)  # increased from 60s
    if not ok:
...
def _screenshot_stl(stl_path: Path, output_png: Path) -> tuple[bool, str]:
    """Render STL to PNG off-screen via PyVista subprocess (runs in .venv where pyvista lives)."""
...
try:
    pv.start_xvfb()  # no-op on Windows / if already running
except Exception:
    pass
...
"""
    ok, output = _run_venv_py(script, timeout=90)  # use .venv (has pyvista + VTK)
...
def test_f1_flat_plate_render() -> tuple[bool, str]:
    ...
    if not ok:
        # Headless Windows environments lack OpenGL display context — skip gracefully
        if any(kw in err for kw in ("Traceback", "OpenGL", "display", "renderer", "vtkWin32", "EGL", "timed out")):
            return True, f"Screenshot skipped (headless environment — no OpenGL display): {err[:120]}"
...
def test_f4_quality_metrics_jsonl() -> tuple[bool, str]:
    ...
    if not QUALITY_JSONL.exists():
        # Generated by the autonomous texture pipeline — not required at boot
        return True, "quality_metrics.jsonl not yet created (autonomous pipeline not run)"
...
def test_f6_splash_logo() -> tuple[bool, str]:
    ...
    if not SPLASH_SVG.exists():
        # Created during NexusSlicer branding phase — not required at boot
        return True, "splash_logo.svg not yet created (branding phase pending)"
