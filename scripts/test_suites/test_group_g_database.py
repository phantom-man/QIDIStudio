...
# Primary LanceDB knowledge table (memory/extract.py writes here)
LANCEDB_PRIMARY_TABLE = "qidistudio_learnings"
LANCEDB_FALLBACK_TABLE = "documents"  # legacy name


def test_g1_lancedb_row_count() -> tuple[bool, str]:
    """LanceDB primary knowledge table has ≥ 100 rows."""
    script = f"""
import sys, os
...
primary = "qidistudio_learnings"
fallback = "documents"
tname = primary if primary in tables else (fallback if fallback in tables else None)
if tname:
...
"""
    ...
    if "ROWS:" in output:
        ...
        tbl_lines = [l for l in output.splitlines() if l.startswith("TABLE:")]
        tbl = tbl_lines[-1].split(":", 1)[1] if tbl_lines else "unknown"
        if rows >= 100:
            return True, f"LanceDB {tbl}: {rows} rows"
...
def test_g2_lancedb_tables() -> tuple[bool, str]:
    """LanceDB has at least one knowledge table (qidistudio_learnings, cnc_parts, or documents)."""
    script = f"""
import sys, os
sys.path.insert(0, r"{REPO_ROOT}")
from dotenv import load_dotenv; load_dotenv(r"{REPO_ROOT}/.env", override=True)
...
"""
    ...
    if "TABLES:" in output:
        ...
        # Accept any of the known table names
        known = {"qidistudio_learnings", "cnc_parts", "documents"}
        found = [t for t in tables if t in known]
        if found:
            return True, f"Knowledge tables: {tables}"
        return False, f"No known tables found. Tables: {tables}"
...
"""
    ...
    if "MISSING:" in output:
        ...
        if not missing:
            ...
            count_info = (
                f" ({counts[-1].split(':')[1]} prompts stored)" if counts else ""
            )
...
def test_g6_quality_metrics_jsonl() -> tuple[bool, str]:
    """quality_metrics.jsonl exists and contains ≥ 5 metric entries (created by autonomous pipeline)."""
    if not QUALITY_JSONL.exists():
        # This file is generated by the autonomous texture/quality pipeline — not required at boot
        return (
            True,
            "quality_metrics.jsonl not yet created (autonomous pipeline not yet run)",
        )
    ...
    if records:
        return (
            False,
            f"Only {len(records)} entries (expected ≥ 5). Consider running the autonomous pipeline.",
        )
...
db = lancedb.connect(uri)
tables = db.table_names()
tname = "qidistudio_learnings" if "qidistudio_learnings" in tables else ("documents" if "documents" in tables else None)
if not tname:
...
TESTS: list[tuple[str, str, callable]] = [
    (
        "G.lancedb_row_count",
        "LanceDB knowledge table \u2265 100 rows",
        test_g1_lancedb_row_count,
    ),
    ("G.lancedb_tables", "LanceDB has knowledge tables", test_g2_lancedb_tables),
    (
        "G.pg_langgraph_tables",
        "Postgres: 3 LangGraph checkpoint tables",
        test_g3_postgres_langgraph_tables,
    ),
    ...
    (
        "G.quality_jsonl",
        "quality_metrics.jsonl ≥ 5 entries",
        test_g6_quality_metrics_jsonl,
    ),
