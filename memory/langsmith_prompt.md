...
Learnings are now stored in two canonical locations — **not in this table**:

1. **`memory/session_learnings_archive.md`** — append-only archive of all verified rows
2. **LanceDB (GCS)** — indexed by `memory/extract.py`, retrieved via `inject.py --query`

To add new learnings: append rows to `.github/copilot-instructions.md` Session Learnings Log.
That file is pruned automatically by `extract.py` (prune→index order) after each run.

Searching: `memory_env\Scripts\python.exe memory/inject.py --query "your topic"`

<!-- All rows permanently moved to memory/session_learnings_archive.md -->

                                                                                                                                                                                                                               | Rationale                                                                                                                                                                 |

| ---------- | ---------------- | ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ---------- | ---------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| 2026-02-27 | bpy*pipeline | calc_normals_split removed Blender 4.1 | Do not call mesh.calc_normals_split(); use poly.normal directly | Removed in Blender 4.1; causes AttributeError mid-pipeline |
...
| 2026-02-28 | implementation | .clang-tidy IMPLEMENTED | `.clang-tidy` created with bugprone-use-after-move, bugprone-suspicious-memset as WarningsAsErrors; modernize-* + performance-_ as warnings; HeaderFilterRegex targets only src/libslic3r + src/slic3r | Tier 1 (errors) vs Tier 2 (warnings) split prevents noisy CI while catching real bugs |
...
| 2026-02-28 | tooling          | Desktop Commander MCP integrated           | DC MCP v0.2.37 registered in VS Code toolset as `mcp*desktop-comma*\*`; cloned upstream at `DesktopCommanderMCP/`(gitignored); PowerShell shell — use`;`not`&&`; docs in `docs/DESKTOP_COMMANDER_MCP.md`                           | Enables direct terminal access for build/test/git; build dir =`C:\QIDISrc\QIDIStudio\build`                                                                             |
