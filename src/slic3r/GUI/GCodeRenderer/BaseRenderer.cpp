    ...
    namespace GUI
    {
        namespace gcode
        {
            ...
            void BaseRenderer::load_shells(const Print &print, bool initialized, bool force_previewing)
            {
                ...
                while (true)
                {
                    GLVolumePtrs::iterator it = std::find_if(m_shells.volumes.volumes.begin(), m_shells.volumes.volumes.end(), [](GLVolume *volume)
                ...
                BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(": shell loaded, id change to %1%, modify_count %2%, object count %3%, glvolume count %4%") % m_shells.print_id % m_shells.print_modify_count % object_count % m_shells.volumes.volumes.size();
            }
            ...
            void BaseRenderer::load(const GCodeProcessorResult &gcode_result, const Print &print, const BuildVolume &build_volume, const std::vector<BoundingBoxf3> &exclude_bounding_box, bool initialized, ConfigOptionMode mode, bool only_gcode)
            {
                ...
                else if (&wxGetApp() && wxGetApp().app_config->get_bool("show_shells_in_preview"))
                { // QDS: load shell at helio_gcode
            ...
            void BaseRenderer::render_all_plates_stats(const std::vector<const GCodeProcessorResult *> &gcode_result_list, bool show) const
            {
                ...
                // item
                {
                    ...
                    // Calculating Column Offsets for the Time Estimation Table
                    {
                        ...
                        for (int i = 0; i < time_offsets.size(); i++)
                        {
                            time_est_table_offsets[time_columns[i].first] = time_offsets[i];
                        }
                    }
       ...
       // QDS: add mutex for protection of gcode result
            ...
            void BaseRenderer::render_sequential_view(uint32_t canvas_width, uint32_t canvas_height, int right_margin)
            {
                ...
                if (show_sequential_view())
                {
                    ...
                    p_sequential_view->render(m_legend_height, canvas_width, canvas_height - bottom_margin * m_scale, right_margin * m_scale, m_view_type, [this](size_t &length_of_line) -> void
                                              {
                        length_of_line = 90;
                        this->set_show_horizontal_slider(true); }, is_show_horizontal_slider(), is_helio_option());
                }
            }
            ...
            void BaseRenderer::render_legend(float &legend_height, int canvas_width, int canvas_height, int right_margin)
            {
                ...
                {
                    auto it = std::find_if(time_mode.roles_times.begin(), time_mode.roles_times.end(), [role](const std::pair<ExtrusionRole, float> &item)
                                           { return role == item.first; });
                ...
                {
                    auto it = std::find_if(time_mode.moves_times.begin(), time_mode.moves_times.end(), [move_type](const std::pair<EMoveType, float> &item)
                ...
                {
                    imgui.title(_u8L("Thermal Index (min)"));
                    break;
                }
                case EViewType::ThermalIndexMax:
                {
                    imgui.title(_u8L("Thermal Index (max)"));
                    break;
                }
                case EViewType::ThermalIndexMean:
                {
                    imgui.title(_u8L("Thermal Index (mean)"));
                    break;
                }
                // end helio
                default:
                ...
                case EViewType::Height:
                {
                    append_range(m_p_extrusions->ranges.height, 2);
                    break;
                }
                case EViewType::Width:
                {
                    append_range(m_p_extrusions->ranges.width, 2);
                    break;
                }
                case EViewType::Feedrate:
                {
                ...
                case EViewType::FanSpeed:
                {
                    append_range(m_p_extrusions->ranges.fan_speed, 0);
                    break;
                }
                case EViewType::Temperature:
                {
                    append_range(m_p_extrusions->ranges.temperature, 0);
                    break;
                }
                case EViewType::LayerTime:
                {
                    append_range(m_p_extrusions->ranges.layer_duration, 1);
                    break;
                }
                case EViewType::VolumetricRate:
                {
                    append_range(m_p_extrusions->ranges.volumetric_rate, 2);
                    break;
                }
                case EViewType::Tool:
                {
                ...
                if (m_view_type == EViewType::ColorPrint)
                {
                    ...
                    auto generate_partial_times = [this, get_used_filament_from_volume](const TimesList &times, const std::vector<double> &used_filaments)
                    {
                        ...
                        for (const auto &time_rec : times)
                        {
                            ...
                            {
                                auto it = std::find_if(custom_gcode_per_print_z.begin(), custom_gcode_per_print_z.end(), [time_rec](const CustomGCode::Item &item)
                ...
                ImGui::BeginChild("#AMS", ImVec2(0, AMS_container_height), true, ImGuiWindowFlags_AlwaysUseWindowPadding);
                {
                    ...
                    if (any_more_to_best)
                    {
                        ...
                        if (delta_weight_to_best >= 0 && delta_change_to_best >= 0)
                            tip = from_u8((boost::format(_u8L("Increase %1%g filament and %2% changes compared to optimal grouping.")) % number_format(delta_weight_to_best) % delta_change_to_best).str());
                        else if (delta_weight_to_best >= 0 && delta_change_to_best < 0)
                            tip = from_u8((boost::format(_u8L("Increase %1%g filament and save %2% changes compared to optimal grouping.")) % number_format(delta_weight_to_best) % std::abs(delta_change_to_best)).str());
                        else if (delta_weight_to_best < 0 && delta_change_to_best >= 0)
                            tip = from_u8((boost::format(_u8L("Save %1%g filament and increase %2% changes compared to optimal grouping.")) % number_format(std::abs(delta_weight_to_best)) % delta_change_to_best).str());
                    ...
                    else if (any_less_to_single_ext)
                    {
                        ...
                        if (delta_weight_to_single_ext >= 0 && delta_change_to_single_ext >= 0)
                            tip = from_u8((boost::format(_u8L("Save %1%g filament and %2% changes compared to a printer with one nozzle.")) % number_format(delta_weight_to_single_ext) % delta_change_to_single_ext).str());
                        else if (delta_weight_to_single_ext >= 0 && delta_change_to_single_ext < 0)
                            tip = from_u8((boost::format(_u8L("Save %1%g filament and increase %2% changes compared to a printer with one nozzle.")) % number_format(delta_weight_to_single_ext) % std::abs(delta_change_to_single_ext)).str());
                        else if (delta_weight_to_single_ext < 0 && delta_change_to_single_ext >= 0)
                            tip = from_u8((boost::format(_u8L("Increase %1%g filament and save %2% changes compared to a printer with one nozzle.")) % number_format(std::abs(delta_weight_to_single_ext)) % delta_change_to_single_ext).str());
                        imgui.text_wrapped(tip, parent_width);
                    }
                    ImGui::Dummy({window_padding, window_padding});
                    if (!is_optimal_group)
            ...
            void BaseRenderer::update_moves_slider(bool set_to_max)
            {
                ...
                if (p_sequential_view->endpoints.last < p_sequential_view->endpoints.first)
                    return;
            ...
            {
                if (view_type_items.empty())
                {
                    return;
                }
                if (m_last_non_helio_option_item >= (int)view_type_items.size())
                {
                    return;
                }
                EViewType cur_type = m_nozzle_nums > 1 ? EViewType::Summary : EViewType::ColorPrint;
                if (m_last_non_helio_option_item < 0)
                { // not set
                ...
                std::vector<GCodeProcessor::ThermalIndex> thermal_indexes, float top, float right, float wnd_height, float f_lines_count, uint64_t start_id, uint64_t end_id) const
            {
                ...
                auto place_window = [text_height, thermal_indexes, top, wnd_height, f_lines_count, start_id, end_id](std::string heading, size_t index_id, float right)
                {
                    ...
                    for (uint64_t id = start_id; id <= end_id; ++id)
                    {
                        ...
                        {
                        case 0:
                            ti_value = thermal_index.min;
                            break;
                        case 1:
                            ti_value = thermal_index.max;
                            break;
                        case 2:
                            ti_value = thermal_index.mean;
                            break;
                        };
            ...
            void SequentialView::Marker::render(int canvas_width, int canvas_height, const EViewType &view_type) const
            {
                ...
                shader->set_uniform("view_matrix", camera.get_view_matrix());
