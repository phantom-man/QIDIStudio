...
#define ENABLE_CALIBRATION_THUMBNAIL_OUTPUT 0
                        ...
                        auto is_one_extruder_printed_model = [objects, extruder]()
                        {
                            for (ModelObject *object : objects)
                            {
                                ...
                                for (ModelVolume *volume : object->volumes)
                                    if ((volume->config.has("extruder") && volume->config.option("extruder")->getInt() != extruder) || !volume->mmu_segmentation_facets.empty())
                                        return false;
            ...
            bool LegacyRenderer::load_toolpaths(const GCodeProcessorResult &gcode_result, const BuildVolume &build_volume, const std::vector<BoundingBoxf3> &exclude_bounding_box)
            {
                ...
                ProgressDialog *progress_dialog = m_only_gcode_in_preview ? new ProgressDialog(_L("Loading G-codes"), "...",
                                                                                               100, wxGetApp().mainframe, wxPD_AUTO_HIDE | wxPD_APP_MODAL)
                                                                          : nullptr;
                wxBusyCursor busy;
                ...
                for (size_t i = 0; i < m_moves_count; ++i)
                {
                    ...
                    {
                        add_vertices_as_point(curr, v_buffer);
                        break;
                    }
       ...
       // dismiss, no more needed
                ...
                if (!options_zs.empty())
                {
                    ...
                    for (Path &path : extrude_buffer.paths)
                    {
                        ...
                        if (std::find_if(options_zs.begin(), options_zs.end(), [z](float f)
                                         { return f - EPSILON <= z && z <= f + EPSILON; }) != options_zs.end())
            ...
            void LegacyRenderer::refresh_render_paths(bool keep_sequential_current_first, bool keep_sequential_current_last) const
            {
                ...
                auto extrusion_color = [this](const Path &path)
                {
                    ...
                    {
                        color = m_p_extrusions->ranges.height.get_color_at(path.height);
                        break;
                    }
                    case EViewType::Width:
                    {
                        color = m_p_extrusions->ranges.width.get_color_at(path.width);
                        break;
                    }
                    case EViewType::Feedrate:
                    {
                        color = m_p_extrusions->ranges.feedrate.get_color_at(path.feedrate);
                        break;
                    }
                    case EViewType::FanSpeed:
                    {
                        color = m_p_extrusions->ranges.fan_speed.get_color_at(path.fan_speed);
                        break;
                    }
                    case EViewType::Temperature:
                    {
                        color = m_p_extrusions->ranges.temperature.get_color_at(path.temperature);
                        break;
                    }
                    case EViewType::LayerTime:
                    {
                        color = m_p_extrusions->ranges.layer_duration.get_color_at(path.layer_time, Range::EType::Logarithmic);
                        break;
                    }
                    case EViewType::VolumetricRate:
                    {
                        color = m_p_extrusions->ranges.volumetric_rate.get_color_at(path.volumetric_rate);
                        break;
                    }
                    case EViewType::Tool:
                    {
                    ...
                    {
                        color = m_p_extrusions->ranges.thermal_index_min.get_color_at(path.thermal_index_min);
                        break;
                    }
                    case EViewType::ThermalIndexMax:
                    {
                        color = m_p_extrusions->ranges.thermal_index_max.get_color_at(path.thermal_index_max);
                        break;
                    }
                    case EViewType::ThermalIndexMean:
                    {
                        color = m_p_extrusions->ranges.thermal_index_mean.get_color_at(path.thermal_index_mean);
                        break;
                    }
                    // end helio
                    default:
                ...
                {
                    return (path.delta_extruder < 0.0f) ? Travel_Colors[2] /* Retract */ : ((path.delta_extruder > 0.0f) ? Travel_Colors[1] /* Extrude */ : Travel_Colors[0] /* Move */);
                };
                ...
                if (top_layer_only || !keep_sequential_current_first)
                    p_sequential_view->current.first = 0;
                // QDS
                if (!keep_sequential_current_last)
                    p_sequential_view->current.last = p_sequential_view->gcode_ids.size();
                ...
                for (size_t b = 0; b < m_buffers.size(); ++b)
                {
                    ...
                    buffer->render_paths.erase(std::remove_if(buffer->render_paths.begin(), buffer->render_paths.end(),
                                                              [](const auto &path)
                                                              { return path.sizes.empty() || path.offsets.empty(); }),
            ...
            void LegacyRenderer::render_toolpaths()
            {
                ...
                float near_plane_height = camera.get_type() == Camera::EType::Perspective ? static_cast<float>(viewport[3]) / (2.0f * static_cast<float>(2.0 * std::tan(0.5 * Geometry::deg2rad(camera.get_fov())))) : static_cast<float>(viewport[3]) * 0.0005;
                ...
                auto add_memory = [this, &imgui](const std::string &label, int64_t memory)
                {
                    auto format_string = [memory](const std::string &units, float value)
                    {
                        return std::to_string(memory) + " bytes (" +
                               Slic3r::float_to_string_decimal_point(float(memory) * value, 3) + " " + units + ")";
                    };
