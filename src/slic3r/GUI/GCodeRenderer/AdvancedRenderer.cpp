    ...
    namespace GUI
    {
        namespace gcode
        {
            ...
            void AdvancedRenderer::export_toolpaths_to_obj(const char *filename) const
            {
                ...
                auto do_export = [&](const std::vector<float> &seg_list, const std::vector<PositionData> &position_data_list, const std::vector<SegmentVertex> &segment_vertex_list, bool b_top_layer) -> void
                {
                ...
                // export top layer
                do_export(m_p_layer_manager->get_transient_other_segment_list(), (*m_p_layer_manager)[top_layer_index].get_position_data(), (*m_p_layer_manager)[top_layer_index].get_segment_vertices(), true);
       ...
       // QDS render slider
            ...
            void AdvancedRenderer::load_layer_info(const GCodeProcessorResult &gcode_result, const BuildVolume &build_volume, const std::vector<BoundingBoxf3> &exclude_bounding_box)
            {
                ...
                // QDS: add only gcode mode
                ProgressDialog *progress_dialog = m_only_gcode_in_preview ? new ProgressDialog(_L("Loading G-codes"), "...", 100, wxGetApp().mainframe, wxPD_AUTO_HIDE | wxPD_APP_MODAL) : nullptr;
            ...
            void AdvancedRenderer::do_render_others(const std::vector<uint32_t> &layer_index_list, bool top_layer_only)
            {
                ...
                p_shader->set_uniform("view_matrix", view_matrix);
