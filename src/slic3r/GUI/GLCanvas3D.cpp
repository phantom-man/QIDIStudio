...
#undef Convex
        ...
        void GLCanvas3D::LayersEditing::render_volumes(const GLCanvas3D &canvas, const GLVolumeCollection &volumes) // render volume and layer height texture (has mapping relation with each other)
        {
            ...
            ScopeGuard guard([current_shader]()
                             { if (current_shader != nullptr) wxGetApp().bind_shader(current_shader); });
            ...
            for (GLVolume *glvolume : volumes.volumes)
            {
                ...
                shader->set_uniform("view_matrix", view_matrix); // GPU computes normal matrix
        ...
        GLCanvas3D::Mouse::Drag::Drag()
            : start_position_2D(Invalid_2D_Point), start_position_3D(Invalid_3D_Point), move_volume_idx(-1), move_requires_threshold(false), move_start_threshold_position_2D(Invalid_2D_Point)
        {
        }
        ...
        void GLCanvas3D::SequentialPrintClearance::set_polygons(const Polygons &polygons, const std::vector<std::pair<Polygon, float>> &height_polygons)
        {
            ...
            if (!height_polygons.empty())
            {
                ...
                {
                    height_triangles_count += poly.first.points.size() - 2;
                }
        ...
        GLCanvas3D::GLCanvas3D(wxGLCanvas *canvas, Bed3D &bed)
            : m_canvas(canvas), m_context(nullptr), m_bed(bed)
#if ENABLE_RETINA_GL
              ,
              m_retina_helper(nullptr)
#endif
              ,
              m_in_render(false), m_return_toolbar(), m_canvas_type(ECanvasType::CanvasView3D), m_gizmos(*this), m_use_clipping_planes(false), m_sidebar_field(""), m_extra_frame_requested(false), m_config(nullptr), m_process(nullptr), m_model(nullptr), m_dirty(true), m_initialized(false), m_apply_zoom_to_volumes_filter(false), m_picking_enabled(false), m_moving_enabled(false), m_dynamic_background_enabled(false), m_multisample_allowed(false), m_moving(false), m_tab_down(false), m_cursor_type(Standard), m_color_by("volume"), m_reload_delayed(false)
#if ENABLE_RENDER_PICKING_PASS
              ,
              m_show_picking_texture(false)
#endif // ENABLE_RENDER_PICKING_PASS
              ,
              m_render_sla_auxiliaries(true), m_layers_editing(*this), m_labels(*this), m_slope(m_volumes), m_sequential_print_clearance(*this)
        {
            if (m_canvas != nullptr)
            {
                ...
                if (gl_volume)
                {
                    ...
                    for (int i = 1; i < volume_ids.size(); i++)
                    {
                        ...
                        if (temp_volume)
                        {
                            ...
                            if (object_sets.find(temp_object_idx) == object_sets.end())
                            {
                                return false;
                            }
                        }
        ...
        static std::pair<bool, bool> construct_extruder_unprintable_error(ObjectFilamentResults &object_result, std::string &left_extruder_unprintable_text, std::string &right_extruder_unprintable_text)
        {
            ...
            for (size_t idx = 0; idx < tips.size(); ++idx)
            {
                ...
                if ((idx == 0 && left_unprintable_objects.size() > 1) || (idx == 1 && right_unprintable_objects.size() > 1))
                    ...
                    model_prefix = (boost::format(_u8L("The position or size of the model %s exceeds the %s's printable range.")) % object_result.object_filaments.front().object->name % nozzle_name).str();
                ...
                if (idx == 0)
                    tips[idx] += (boost::format(_u8L("Left nozzle: X:%1%-%2%, Y:%3%-%4%, Z:%5%-%6%\n")) % format_number(left_x_min) % format_number(left_x_max) % format_number(left_y_min) % format_number(left_y_max) % format_number(left_z_min) % format_number(left_z_max)).str();
                else
                    tips[idx] += (boost::format(_u8L("Right nozzle: X:%1%-%2%, Y:%3%-%4%, Z:%5%-%6%")) % format_number(right_x_min) % format_number(right_x_max) % format_number(right_y_min) % format_number(right_y_max) % format_number(right_z_min) % format_number(right_z_max)).str();
                ...
                else
                {
                    if ((mo == nullptr || m_model->objects[vol->composite_id.object_id] == mo) && (instance_idx == -1 || vol->composite_id.instance_id == instance_idx) && (mv == nullptr || m_model->objects[vol->composite_id.object_id]->volumes[vol->composite_id.volume_id] == mv))
                    {
                        ...
                        else
                        {
                            ...
                            if (m_gizmos.is_paint_gizmo() && !vol->is_modifier)
            ...
            for (const GLVolume *volume : m_volumes.volumes)
            {
                if (!m_apply_zoom_to_volumes_filter || ((volume != nullptr) && volume->zoom_to_volumes))
                {
                    ...
                    if (is_limit && !expand_part_plate_list_box.overlap(v_bb))
                        continue;
                    if (v_bb.max_size() > 100000)
                    { // unit::mm more than 100m
                        continue;
                    }
                    bb.merge(v_bb);
                }
        ...
        void GLCanvas3D::render(bool only_init)
        {
            ...
            m_in_render = true;
            Slic3r::ScopeGuard in_render_guard([this]()
                                               { m_in_render = false; });
       ...
       // draw scene
                ...
                for (const SLAPrintObject *print_object : sla_print->objects())
                {
                    ...
                    for (size_t print_instance_idx = 0; print_instance_idx < print_object->instances().size(); ++print_instance_idx)
                    {
                        ...
                        auto it = std::find_if(model_object->instances.begin(), model_object->instances.end(),
                                               [&instance](const ModelInstance *mi)
                                               { return mi->id() == instance.instance_id; });
        ...
        void GLCanvas3D::bind_event_handlers()
        {
            if (m_canvas != nullptr)
            {
                ...
                m_canvas->Bind(EVT_GLCANVAS_TOOLBAR_HIGHLIGHTER_TIMER, [this](wxTimerEvent &)
                               { m_toolbar_highlighter.blink(); });
                m_gizmo_highlighter.set_timer_owner(m_canvas, 0);
                m_canvas->Bind(EVT_GLCANVAS_GIZMO_HIGHLIGHTER_TIMER, [this](wxTimerEvent &)
        ...
        void GLCanvas3D::on_char(wxKeyEvent &evt)
        {
            ...
            if (keyCode == WXK_ESCAPE && (_deactivate_arrange_menu() || _deactivate_orient_menu() || _deactivate_layersediting_menu()))
                return;
                            ...
                            if (m_canvas_type == CanvasPreview)
                            {
                                ...
                                if (evt.CmdDown() || evt.ShiftDown())
                                {
                                    ...
                                    if (keyCode == WXK_UP || keyCode == WXK_DOWN)
                                    {
                                        ...
                                        if (m_layers_slider->is_one_layer())
...
// #define SLIC3R_DEBUG_MOUSE_EVENTS
    ...
    if (wxGetApp().plater()->get_view_toolbar().on_mouse(evt, *this)) {
        ...
        return;
            ...
            else if (evt.LeftDown() || evt.RightDown() || evt.MiddleDown())
            {
                ...
                else
                {
                    ...
                    {
                        if (m_gizmos.get_current_type() != GLGizmosManager::SlaSupports && !m_gizmos.is_paint_gizmo())
                        {
        ...
        std::vector<Vec2f> GLCanvas3D::get_empty_cells(const Vec2f start_point, const Vec2f step)
        {
            ...
            std::sort(cells.begin(), cells.end(), [start](const Vec2f &cell1, const Vec2f &cell2)
                      { return (cell1 - start).norm() < (cell2 - start).norm(); });
            return cells;
        }
    ...
    if (!_init_view_toolbar())
        ...
        bool GLCanvas3D::_init_main_toolbar()
        {
            ...
            if (m_canvas_type == ECanvasType::CanvasView3D)
            {
                ...
                { if (m_canvas != nullptr) wxPostEvent(m_canvas, SimpleEvent(EVT_GLTOOLBAR_ADD)); };
                item.enabling_callback = []() -> bool
                { return wxGetApp().plater()->can_add_model(); };
                ...
                item.left.action_callback = [this]()
                { if (m_canvas != nullptr) wxPostEvent(m_canvas, SimpleEvent(EVT_GLTOOLBAR_ADD_PLATE)); };
                item.enabling_callback = []() -> bool
                ...
                item.enabling_callback = []() -> bool
                { return wxGetApp().plater()->can_do_ui_job(); };
                ...
                item.left.action_callback = [this]()
                {
                    if (m_canvas != nullptr)
                    {
                        NetworkAgent *agent = GUI::wxGetApp().getAgent();
                        if (agent)
                            agent->track_update_property("auto_arrange", std::to_string(++auto_arrange_count));
                    }
                };
                item.enabling_callback = []() -> bool
                { return wxGetApp().plater()->can_do_ui_job(); };
                ...
                layers_editing_item.left.action_callback = [this]() -> void
                {
                    if (m_canvas != nullptr)
                    {
                        ...
                        NetworkAgent *agent = GUI::wxGetApp().getAgent();
                        if (agent)
                            agent->track_update_property("custom_height", std::to_string(++custom_height_count));
                    }
                };
                ...
                layers_editing_item.enabling_callback = []() -> bool
...

                {
                    ...
                    sperate_item.visibility_callback = []() -> bool
                    { return true; };
                    sperate_item.enabling_callback = []() -> bool
                    { return true; };
                ...
                if (m_canvas_type == ECanvasType::CanvasView3D)
                {
                    ...
                    item.left.action_callback = [this]()
                    {
                        if (m_canvas != nullptr)
                        {
                            ...
                            NetworkAgent *agent = GUI::wxGetApp().getAgent();
                            if (agent)
                                agent->track_update_property("split_to_objects", std::to_string(++split_to_objects_count));
                        }
                    };
                    ...
                    item.left.action_callback = [this]()
                    {
                        if (m_canvas != nullptr)
                        {
                            ...
                            NetworkAgent *agent = GUI::wxGetApp().getAgent();
                            if (agent)
                                agent->track_update_property("split_to_part", std::to_string(++split_to_part_count));
                        }
                    };
                    item.visibility_callback = GLToolbarItem::Default_Visibility_Callback;
                    item.enabling_callback = []() -> bool
            ...
            if (m_canvas_type == ECanvasType::CanvasView3D)
            {
                ...
                more_item.enabling_callback = []() -> bool
                { return true; };
...
#define USE_PARALLEL 1
       ...
       // Update collapse toolbar
        ...
        void GLCanvas3D::_render_return_toolbar()
        {
            ...
            imgui.begin(_L("Assembly Return"), ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse);
        ...
        void GLCanvas3D::_render_assemble_control()
        {
            ...
            }
            {
                ...
                if (selection_idx != selection_out)
                { // do
                    if (selection_out == 0)
                    {
        ...
        void GLCanvas3D::_render_camera_target() const
        {
            ...
            model_matrix = Geometry::translation_transform(target.cast<double>()) * Geometry::rotation_transform({0.0f, 0.0f, 0.5 * PI}) * Geometry::scale_transform({scale, 1.0f, 1.0f});

            p_flat_shader->set_uniform("view_model_matrix", view_matrix * model_matrix);
            m_camera_target_mark.set_color({0.0f, 1.0f, 0.0f, 1.0f});
            m_camera_target_mark.render_geometry();

            model_matrix = Geometry::translation_transform(target.cast<double>()) * Geometry::rotation_transform({0.0f, -0.5 * PI, 0.0f}) * Geometry::scale_transform({scale, 1.0f, 1.0f});
        ...
        void GLCanvas3D::_perform_layer_editing_action(wxMouseEvent *evt)
        {
            ...
            if (evt != nullptr)
            {
                ...
                m_layers_editing.last_action =
                    evt->ShiftDown() ? (evt->RightIsDown() ? LAYER_HEIGHT_EDIT_ACTION_SMOOTH : LAYER_HEIGHT_EDIT_ACTION_REDUCE) : (evt->RightIsDown() ? LAYER_HEIGHT_EDIT_ACTION_INCREASE : LAYER_HEIGHT_EDIT_ACTION_DECREASE);
            }
        ...
        void GLCanvas3D::_load_print_toolpaths(const BuildVolume &build_volume)
        {
            ...
            const PrintObject *highest_object = *std::max_element(print->objects().begin(), print->objects().end(), [](auto l, auto r)
        ...
        void GLCanvas3D::_load_print_object_toolpaths(const PrintObject &print_object, const BuildVolume &build_volume, const std::vector<std::string> &str_tool_colors, const std::vector<CustomGCode::Item> &color_print_values)
        {
            ...
            struct Ctxt
            {
                ...
                static const std::array<float, 4> &color_perimeters()
                {
                    static std::array<float, 4> color = {1.0f, 1.0f, 0.0f, 1.f};
                    return color;
                } // yellow
                static const std::array<float, 4> &color_infill()
                {
                    static std::array<float, 4> color = {1.0f, 0.5f, 0.5f, 1.f};
                    return color;
                } // redish
                static const std::array<float, 4> &color_support()
                {
                    static std::array<float, 4> color = {0.5f, 1.0f, 0.5f, 1.f};
                    return color;
                } // greenish
                static const std::array<float, 4> &color_pause_or_custom_code()
                {
                    static std::array<float, 4> color = {0.5f, 0.5f, 0.5f, 1.f};
                    return color;
                } // gray
            ...
            if (ctxt.has_support)
                for (const Layer *layer : print_object.support_layers())
                    ctxt.layers.emplace_back(layer);
            std::sort(ctxt.layers.begin(), ctxt.layers.end(), [](const Layer *l1, const Layer *l2)
            ...
            tbb::parallel_for(
                ...
                [&ctxt, &new_volume, is_selected_separate_extruder, this](const tbb::blocked_range<size_t> &range)
                {
                    ...
                    {
                        return *vols[ctxt.color_by_color_print() ? ctxt.color_print_color_idx_by_layer_idx_and_extruder(layer_idx, extruder) : ctxt.color_by_tool() ? std::min<int>(ctxt.number_tools() - 1, std::max<int>(extruder - 1, 0))
                                                                                                                                                                    : feature];
                    };
                                    ...
                                    for (const ExtrusionEntity *ee : layerm->fills.entities)
                                    {
                                        ...
                                        if (!fill->entities.empty())
                                            _3DScene::extrusionentity_to_verts(*fill, float(layer->print_z), copy,
                                                                               volume(idx_layer,
                                                                                      is_solid_infill(fill->entities.front()->role()) ? layerm->region().config().solid_infill_filament : layerm->region().config().sparse_infill_filament,
                                                                                      1));
                                    }
                                }
                            }
        ...
        void GLCanvas3D::_load_wipe_tower_toolpaths(const BuildVolume &build_volume, const std::vector<std::string> &str_tool_colors)
        {
            ...
            struct Ctxt
            {
                ...
                const std::vector<WipeTower::ToolChangeResult> &tool_change(size_t idx)
                {
                    ...
                    return priming.empty() ? ((idx == tool_changes.size()) ? final : tool_changes[idx]) : ((idx == 0) ? priming : (idx == tool_changes.size() + 1) ? final
                                                                                                                                                                   : tool_changes[idx - 1]);
                }
            ...
            else
            {
                if (wxGetApp().is_editor())
                {
                    if (current_printer_technology() != ptSLA)
                    {
                        ...
                        // w18
                        if (warning == EWarning::ToolHeightOutside)
                        { // check if max z_layer height exceed max print height
                        ...
                        else if (warning == EWarning::ToolpathOutside)
                        { // check if max x,y coords exceed bed area
                                                    ...
                                                    bool ban_light)
        {
            ...
            if (thumbnail_params.use_plate_box)
            {
                ...
                {
                    if (!vol->is_modifier && !vol->is_wipe_tower && (!thumbnail_params.parts_only || vol->composite_id.volume_id >= 0) && (vol->partly_inside || is_volume_in_plate_boundingbox(*vol, plate_idx, plate_build_volume)))
                    {
        ...
        void GLCanvas3D::_set_warning_notification(EWarning warning, bool state)
        {
            ...
            case EWarning::GCodeConflict:
            {
                ...
                if (!t_conflict_result.has_value())
                {
                    break;
                }
            ...
            case EWarning::SomethingNotShown:
                text = _u8L("Only the object being edited is visible.");
                break;
        ...
        bool GLCanvas3D::is_flushing_matrix_error()
        {
            ...
            {
                if (multiplier == 0 && config_matrix.size() >= config_multiplier.size() * 4)
                    return true;
            }
