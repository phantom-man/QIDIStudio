...
namespace Slic3r::GUI
{
    ...
    void GLGizmoPainterBase::render_triangles(const Selection &selection) const
    {
        ...
        for (const ModelVolume *mv : mo->volumes)
        {
            ...
            shader->set_uniform("view_matrix", camera.get_view_matrix()); // GPU computes normal matrix
    ...
    void GLGizmoPainterBase::render_cursor_height_range(const Transform3d &trafo) const
    {
        ...
        if (m_rr.mouse_position[0] >= m_height_start_pos[0] && m_rr.mouse_position[0] <= m_height_start_pos[0] + imgui_size[0] && m_rr.mouse_position[1] >= m_height_start_pos[1] && m_rr.mouse_position[1] <= m_height_start_pos[1] + imgui_size[1])
        {
        ...
        const std::vector<Transform3d> &trafo_matrices) const
    {
        ...
        const ModelObject *mo = m_c->selection_info()->model_object();
        const ModelInstance *mi = mo->instances[selection.get_instance_idx()];
        const Transform3d instance_trafo = m_parent.get_canvas_type() == GLCanvas3D::CanvasAssembleView ? mi->get_assemble_transformation().get_matrix() : mi->get_transformation().get_matrix();
        const Transform3d instance_trafo_not_translate = m_parent.get_canvas_type() == GLCanvas3D::CanvasAssembleView ? mi->get_assemble_transformation().get_matrix(true) : mi->get_transformation().get_matrix(true);
    ...
    bool GLGizmoPainterBase::gizmo_event(SLAGizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down, bool control_down)
    {
        ...
        if (action == SLAGizmoEventType::MouseWheelUp || action == SLAGizmoEventType::MouseWheelDown)
        {
            if (control_down)
            {
                ...
                {
                    m_cursor_height = action == SLAGizmoEventType::MouseWheelDown ? std::max(m_cursor_height - this->get_cursor_height_step(), this->get_cursor_height_min()) : std::min(m_cursor_height + this->get_cursor_height_step(), this->get_cursor_height_max());
                ...
                if (m_tool_type == ToolType::BUCKET_FILL || m_tool_type == ToolType::SMART_FILL)
                {
                    ...
                    if (m_rr.mesh_id != -1)
                    {
                        ...
                        const ModelObject *mo = m_c->selection_info()->model_object();
                        const ModelInstance *mi = mo->instances[selection.get_instance_idx()];
                        const Transform3d trafo_matrix_not_translate = m_parent.get_canvas_type() == GLCanvas3D::CanvasAssembleView ? mi->get_assemble_transformation().get_matrix(true) * mo->volumes[m_rr.mesh_id]->get_matrix(true) : mi->get_transformation().get_matrix(true) * mo->volumes[m_rr.mesh_id]->get_matrix(true);
                        const Transform3d trafo_matrix = m_parent.get_canvas_type() == GLCanvas3D::CanvasAssembleView ? mi->get_assemble_transformation().get_matrix() * mo->volumes[m_rr.mesh_id]->get_matrix() : mi->get_transformation().get_matrix() * mo->volumes[m_rr.mesh_id]->get_matrix();
                ...
                {
                    TriangleSelectorPatch::gap_area = action == SLAGizmoEventType::MouseWheelDown ? std::max(TriangleSelectorPatch::gap_area - TriangleSelectorPatch::GapAreaStep, TriangleSelectorPatch::GapAreaMin) : std::min(TriangleSelectorPatch::gap_area + TriangleSelectorPatch::GapAreaStep, TriangleSelectorPatch::GapAreaMax);
                    m_parent.set_as_dirty();
                    return true;
                }
            }
        ...
        if (action == SLAGizmoEventType::LeftDown || action == SLAGizmoEventType::RightDown || (action == SLAGizmoEventType::Dragging && m_button_down != Button::None))
        {
            ...
            const ModelObject *mo = m_c->selection_info()->model_object();
            const ModelInstance *mi = mo->instances[selection.get_instance_idx()];
            Transform3d instance_trafo = m_parent.get_canvas_type() == GLCanvas3D::CanvasAssembleView ? mi->get_assemble_transformation().get_matrix() : mi->get_transformation().get_matrix();
            Transform3d instance_trafo_not_translate = m_parent.get_canvas_type() == GLCanvas3D::CanvasAssembleView ? mi->get_assemble_transformation().get_matrix(true) : mi->get_transformation().get_matrix(true);
    ...
    CommonGizmosDataID GLGizmoPainterBase::on_get_requirements() const
    {
        return CommonGizmosDataID(
            int(CommonGizmosDataID::SelectionInfo) | int(CommonGizmosDataID::InstancesHider) | int(CommonGizmosDataID::Raycaster) | int(CommonGizmosDataID::ObjectClipper));
    }
    ...
    void TriangleSelectorGUI::render(ImGuiWrapper *imgui, const Transform3d &matrix, bool render_paint_contour_at_same_time)
    {
        ...
        ScopeGuard guard([shader]()
                         { if (shader) shader->set_uniform("offset_depth_buffer", false); });
        ...
        for (auto &iva : m_iva_seed_fills)
        {
            ...
            const std::array<float, 4> &color = TriangleSelectorGUI::get_seed_fill_color(color_idx == 1 ? enforcers_color : color_idx == 2 ? blockers_color
                                                                                                                                           : GLVolume::NEUTRAL_COLOR);
            iva.set_color(color);
            iva.render_geometry();
        }

        ScopeGuard guard_gouraud([shader]()
                                 { wxGetApp().bind_shader(shader); });
        wxGetApp().unbind_shader();
    ...
    void TriangleSelectorGUI::update_render_data()
    {
        ...
        for (const Triangle &tr : m_triangles)
        {
            ...
            GLModel::Geometry &iva = tr.is_selected_by_seed_fill() ? iva_seed_fills_data[tr_state] : tr.get_state() == EnforcerBlockerType::ENFORCER ? iva_enforcers_data
                                                                                                                                                     : iva_blockers_data;
            int &cnt = tr.is_selected_by_seed_fill() ? seed_fill_cnt[tr_state] : tr.get_state() == EnforcerBlockerType::ENFORCER ? enf_cnt
                                                                                                                                 : blc_cnt;
    ...
    void TriangleSelectorPatch::render(ImGuiWrapper *imgui, const Transform3d &matrix, bool render_paint_contour_at_same_time)
    {
        ...
        ScopeGuard guard_mm_gouraud([shader]()
                                    { wxGetApp().bind_shader(shader); });
        wxGetApp().unbind_shader();
    ...
    void TriangleSelectorPatch::finalize_triangle_indices()
    {
        ...
        assert(std::all_of(m_triangle_indices_VBO_ids.cbegin(), m_triangle_indices_VBO_ids.cend(), [](const auto &ti_VBO_id)
                           { return ti_VBO_id == 0; }));
