...
namespace Slic3r
{
    namespace GUI
    {
        ...
        void GLGizmoSlaSupports::render_points(const Selection &selection, bool picking) const
        {
            ...
            bool has_holes = (!m_c->hollowed_mesh()->get_hollowed_mesh() && !m_c->selection_info()->model_object()->sla_drain_holes.empty());
        ...
        bool GLGizmoSlaSupports::gizmo_event(SLAGizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down, bool control_down)
        {
            ...
            if (m_editing_mode)
            {
                ...
                {
                    ask_about_changes_call_after([this]()
                                                 { editing_mode_apply_changes(); },
                                                 [this]()
        ...
        */
        ...
        RENDER_AGAIN:
            ...
            { // not in editing mode:
                ...
                if (slider_clicked)
                { // stash the values of the settings so we know what to revert to after undo
        ...
        CommonGizmosDataID GLGizmoSlaSupports::on_get_requirements() const
        {
            return CommonGizmosDataID(
                int(CommonGizmosDataID::SelectionInfo) | int(CommonGizmosDataID::InstancesHider) | int(CommonGizmosDataID::Raycaster) | int(CommonGizmosDataID::HollowedMesh) | int(CommonGizmosDataID::ObjectClipper) | int(CommonGizmosDataID::SupportsClipper));
        }
