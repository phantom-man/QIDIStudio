...
namespace Slic3r::GUI
{
    ...
    GLGizmoSimplify::GLGizmoSimplify(GLCanvas3D &parent, unsigned int sprite_id)
        : GLGizmoBase(parent, sprite_id), m_volume(nullptr), m_show_wireframe(false), m_move_to_center(false)
          // translation for GUI size
          ,
          tr_mesh_name(_u8L("Mesh name")), tr_triangles(_u8L("Triangles")), tr_detail_level(_u8L("Detail level")), tr_decimate_ratio(_u8L("Decimate ratio"))
    {
    }
    ...
    void GLGizmoSimplify::process()
    {
        ...
        call_after_if_active([this]() { worker_finished(); }); }, std::move(its));
    }
    ...
    void GLGizmoSimplify::on_render()
    {
        ...
        gouraud_shader->set_uniform("view_matrix", camera.get_view_matrix()); // GPU computes normal matrix
