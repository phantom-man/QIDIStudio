...
namespace Slic3r
{
    namespace GUI
    {
        ...
        Selection::VolumeCache::TransformCache::TransformCache()
            : position(Vec3d::Zero()), rotation(Vec3d::Zero()), scaling_factor(Vec3d::Ones()), mirror(Vec3d::Ones()), rotation_matrix(Transform3d::Identity()), scale_matrix(Transform3d::Identity()), mirror_matrix(Transform3d::Identity()), full_tran(Transform3d::Identity())
        {
        }

        Selection::VolumeCache::TransformCache::TransformCache(const Geometry::Transformation &transform)
            : position(transform.get_offset()), rotation(transform.get_rotation()), scaling_factor(transform.get_scaling_factor()), mirror(transform.get_mirror()), full_tran(transform)
        {
        ...
        Selection::Selection()
            : m_volumes(nullptr), m_model(nullptr), m_enabled(false), m_mode(Instance), m_type(Empty), m_valid(false), m_scale_factor(1.0f), m_dragging(false)
        {
    ...
    wxGetApp().obj_manipul()->reset_cache();
        ...
        void Selection::rotate(const Vec3d &rotation, TransformationType transformation_type)
        {
                ...
                { // this is not the wipe tower
                    ...
                    auto rotate_instance = [this, &rotation, &object_instance_first, rot_axis_max, transformation_type](GLVolume &volume, int i)
                    {
                        ...
                        else
                        {
                            ...
                            Vec3d new_rotation = transformation_type.world() ? Geometry::extract_euler_angles(Geometry::assemble_transform(Vec3d::Zero(), rotation) * m_cache.volumes_data[i].get_instance_rotation_matrix()) : transformation_type.absolute() ? rotation
                                                                                                                                                                                                                                                               : rotation + m_cache.volumes_data[i].get_instance_rotation();
            ...
            {                                                        // it's the wipe tower that's selected and being rotated
        ...
        void Selection::render_sidebar_scale_hints(const std::string &sidebar_field, bool gizmo_uniform_scale, GLShaderProgram &shader, const Transform3d &model_matrix) const
        {
            ...
            auto render_sidebar_scale_hint = [this, uniform_scale](Axis axis, GLShaderProgram &shader, const Transform3d &matrix)
            {
                ...
                shader.set_uniform("view_model_matrix", view_model_matrix);
                shader.set_uniform("view_matrix", wxGetApp().plater()->get_camera().get_view_matrix());
                m_arrow.render_geometry();
        ...
        void Selection::paste_objects_from_clipboard()
        {
            ...
            for (size_t i = 0; i < src_objects.size(); i++)
            {
                ...
                if (shift_all(0) != 0 || shift_all(1) != 0)
                {
                    // QDS: if multiple objects are selected, move them as a whole after copy
                    if (i == 0)
                        empty_cell_all = wxGetApp().plater()->canvas3D()->get_nearest_empty_cell({start_point(0), start_point(1)}, {bbox.size()(0) + 1, bbox.size()(1) + 1});
