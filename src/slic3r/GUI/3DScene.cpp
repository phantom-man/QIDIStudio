...
{
    if (colors[3] < FullyTransparentMaterialThreshold)
    ...
    else if ((colors[0] < BlackThreshold) && (colors[1] < BlackThreshold) && (colors[2] < BlackThreshold))
    { // black filament deal
        std::array<float, 4> new_color;
...
namespace Slic3r
{
    ...
    static void smooth_normals_corner(TriangleMesh &mesh, std::vector<stl_normal> &normals)
    {
        ...
        Eigen::MatrixXd vertices = MapMatrixXfUnaligned(mesh.its.vertices.front().data(),
                                                        Eigen::Index(mesh.its.vertices.size()), 3)
                                       .cast<double>();
        ...
        Eigen::MatrixXd in_normals = MapMatrixXfUnaligned(face_normals.front().data(),
                                                          Eigen::Index(face_normals.size()), 3)
                                         .cast<double>();
    ...
    void GLIndexedVertexArray::load_its_flat_shading(const indexed_triangle_set &its)
    {
        ...
        BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(", this %1%, indices size %2%, vertices %3%, triangles %4% ") % this % its.indices.size() % this->vertices_and_normals_interleaved.size() % this->triangle_indices.size();
    }
    ...
    std::array<std::array<float, 4>, 5> GLVolume::MODEL_COLOR = {{{1.0f, 1.0f, 0.0f, 1.f},
                                                                  {1.0f, 0.5f, 0.5f, 1.f},
                                                                  {0.5f, 1.0f, 0.5f, 1.f},
                                                                  {0.5f, 0.5f, 1.0f, 1.f},
                                                                  {1.0f, 1.0f, 0.0f, 1.f}}};
    ...
    GLVolume::GLVolume(float r, float g, float b, float a, bool create_index_data)
        : m_sla_shift_z(0.0), m_sinking_contours(*this)
          // geometry_id == 0 -> invalid
          ,
          geometry_id(std::pair<size_t, size_t>(0, 0)), extruder_id(0), selected(false), disabled(false), printable(true), visible(true), is_active(true), zoom_to_volumes(true), shader_outside_printer_detection_enabled(false), is_outside(false), partly_inside(false), hover(HS_None), is_modifier(false), slice_error(false), is_wipe_tower(false), is_extrusion_path(false), force_transparent(false), force_native_color(false), force_neutral_color(false), force_sinking_contours(false), picking(false), tverts_range(0, size_t(-1)), qverts_range(0, size_t(-1)), tverts_range_lod(0, size_t(-1)), qverts_range_lod(0, size_t(-1))
    {
        color = {r, g, b, a};
    ...
    void GLVolume::render(const GUI::Camera &camera, const std::vector<std::array<float, 4>> &colors, Model &model, bool with_outline, const std::array<float, 4> &body_color, bool disable_mmu_colors) const
    {
        ...
        auto render_body = [&]()
        {
            ...
            do
            {
                ...
                {
                    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(", this %1%, name %2%, current mmuseg_ts %3%, current color size %4%") % this % this->name % mmuseg_ts % mmuseg_ivas.size();
                    mmuseg_ivas.clear();
                    ...
                    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(", this %1%, name %2%, new mmuseg_ts %3%, new color size %4%") % this % this->name % mmuseg_ts % mmuseg_ivas.size();
                }
            } while (0);
