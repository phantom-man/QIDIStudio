...
namespace Slic3r
{
    ...
    int GLShaderProgram::get_attrib_location(const char *name) const
    {
        ...
        auto it = std::find_if(m_attrib_location_cache.begin(), m_attrib_location_cache.end(), [name](const auto &p)
                               { return p.first == name; });
    ...
    int GLShaderProgram::get_uniform_location(const char *name) const
    {
        ...
        const_cast<GLShaderProgram *>(this)->m_uniform_location_cache.emplace(name, id);
        return id;
    }
