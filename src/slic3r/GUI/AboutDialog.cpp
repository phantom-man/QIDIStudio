...
namespace Slic3r
{
    namespace GUI
    {
        ...
        CopyrightsDialog::CopyrightsDialog()
            : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, from_u8((boost::format("%1% - %2%") % (wxGetApp().is_editor() ? SLIC3R_APP_FULL_NAME : GCODEVIEWER_APP_NAME) % _utf8(L("Portions copyright"))).str()),
        ...
        wxString CopyrightsDialog::get_html_text()
        {
            ...
            for (auto &entry : m_entries)
            {
                ...
                text += wxString::Format(
                    "<a href=\"%s\">%s</a><br/><br/>", entry.link, entry.link);
            }
