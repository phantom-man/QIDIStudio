    ...
    namespace GUI
    {
              ...
              m_mode(mode)
        {
            ...
            if (mode == TextureDialogMode::Apply)
            {
                ...
                m_gallery_btn = new wxButton(this, wxID_ANY, _L("Gallery..."),
                                             wxDefaultPosition, wxDefaultSize,
                                             wxBU_EXACTFIT);
                m_gallery_btn->Bind(wxEVT_BUTTON, &TextureParamsDialog::on_gallery, this);
                file_row->AddSpacer(4);
                file_row->Add(m_gallery_btn, 0, wxALIGN_CENTER_VERTICAL);
            }
            ...
            if (mode == TextureDialogMode::Adjust)
            {
                ...
                for (const auto &b : defs)
                {
                    ...
                    double d = b.delta;
                    btn->Bind(wxEVT_BUTTON, [this, d](wxCommandEvent &)
                              { on_quick_adjust(d); });
        ...
        void TextureParamsDialog::on_gallery(wxCommandEvent & /*evt*/)
        {
            SkinBrowserDialog dlg(this);
            if (dlg.ShowModal() != wxID_OK)
                return;

            m_png_ctrl->SetValue(wxString::FromUTF8(dlg.get_png_path()));

            // Apply the gallery category's suggested tile size and relief as helpful
            // defaults — the user can still tweak them in the spinners.
            m_tile_ctrl->SetValue(dlg.get_tile_mm());
            m_relief_ctrl->SetValue(dlg.get_relief());
        }
        ...
        void TextureParamsDialog::on_browse(wxCommandEvent & /*evt*/)
        {
            ...
            wxFileDialog dlg(this,
                             ...
                             default_dir,
                             wxEmptyString,
                             _L("Image files") + " (*.png;*.jpg;*.jpeg)|*.png;*.jpg;*.jpeg" + "|" + _L("All files") + " (*.*)|*.*",
