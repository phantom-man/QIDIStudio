...
//        if (wParam == RIM_INPUTSINK && plater != nullptr && main_frame->IsActive()) {
        ...
        void GUI_App::post_init()
        {
            ...
            if (!this->init_params->input_files.empty())
            {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << boost::format(", init with input files, size %1%, input_gcode %2%") % this->init_params->input_files.size() % this->init_params->input_gcode;
                    ...
                    boost::starts_with(this->init_params->input_files.front(), "qidistudio://open"))
                {
                    ...
                    {
                        input_str_arr.erase(input_str_arr.begin());
                    }
            ...
            { // G-Code Viewer does not initialize preset_updater.
            ...
            if (!log_addr.empty())
            {
                ...
                if (boost::filesystem::exists(log_folder))
                {
                    ...
                    std::sort(files_vec.begin(), files_vec.end(), [](std::pair<time_t, std::string> &a, std::pair<time_t, std::string> &b)
        ...
        GUI_App::GUI_App()
            : wxApp()
              //, m_app_mode(mode)
              ,
              m_app_mode(EAppMode::Editor), m_em_unit(10), m_imgui(new ImGuiWrapper()), hms_query(new HMSQuery()), m_removable_drive_manager(std::make_unique<RemovableDriveManager>()), m_other_instance_message_handler(std::make_unique<OtherInstanceMessageHandler>())
        {
...
//#endif
        ...
        for (auto d : dialogStack)
            ...
            if (m_last_config_version)
            {
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format("last app version {%1%.%2%.%3%}, current version {%4%.%5%.%6%}") % last_major % last_minor % last_patch % cur_major % cur_minor % cur_patch;
            ...
            if (wxButton *btn = dynamic_cast<wxButton *>(window))
            {
                ...
                else
                {
                    ...
                    if (btn->GetId() == wxID_OK || btn->GetId() == wxID_CANCEL)
                    {
                        ...
                        {
                            btn->SetBackgroundColour(mark ? m_color_selected_btn_bg : highlited ? m_color_highlight_default
...
// QDS
    ...
    preset_bundle->physical_printers.load_printers_from_presets(preset_bundle->printers);
        ...
        wxString GUI_App::transition_tridid(int trid_id) const
        {
            ...
            else if (trid_id >= 0x80 && trid_id <= 0x87)
            { // n3s
...
// TODO: Fix home page not emit get_recent_projects on macOS
                    ...
                    else if (command_str.compare("homepage_leftmenu_switch") == 0)
                    {
                        if (root.get_child_optional("menu") != boost::none)
                        {
                            ...
                            if (mainframe && mainframe->m_webview)
                            {
                                mainframe->m_webview->SwitchLeftMenu(strMenu);
                            }
                        }
                    }
                ...
                if (mainframe && mainframe->m_webview) { mainframe->m_webview->SendMakerlabList(); } });
                    ...
                    else if (command_str.compare("homepage_makerlab_open") == 0)
                    {
                        if (root.get_child_optional("url") != boost::none)
                        {
                            if (wxGetApp().is_user_login())
                            {
                                ...
                                if (mainframe && mainframe->m_webview)
                                {
                                    mainframe->m_webview->OpenOneMakerlab(strUrl);
                                }
                            }
                            else
                            {
                                ...
                                {
                                    CallAfter([this]
                                              { handle_web_request("{\"sequence_id\":1,\"command\":\"homepage_need_networkplugin\"}"); });
                                    return "";
                                }
                                CallAfter([this]
                                          { this->request_login(true); });
                    ...
                    {
                        int NewWidth = 214;
                        if (root.get_child_optional("width") != boost::none)
                            NewWidth = root.get<int>("width");
                    ...
                    else if (command_str.compare("homepage_makerlab_stl_download") == 0)
                    {
                        if (root.get_child_optional("file_data") != boost::none && root.get_child_optional("sequence_id") != boost::none)
                        {
                            ...
                            if (mainframe && mainframe->m_webview)
                            {
                                mainframe->m_webview->SaveMakerlabStl(SeqID, strSTLBase64, strSTLName);
                            }
                        }
                    }
                }
        ...
        void GUI_App::request_open_project(std::string project_id)
        {
            ...
            if (project_id == "<new>")
            {
                ...
                plater()->load_project();
                                      ...
                                      [](char c)
                                      { return std::isdigit(c); }) == project_id.end())
                ;
        ...
        void GUI_App::sync_preset(Preset *preset)
        {
            ...
            if (http_code >= 400 && values_map["code"] == "14")
            { // Limit
        ...
        bool GUI_App::select_language()
        {
            ...
            std::sort(language_infos.begin(), language_infos.end(), [](const wxLanguageInfo *l, const wxLanguageInfo *r)
        ...
        bool GUI_App::load_language(wxString language, bool initial)
        {
            ...
            if (initial)
            {
                ...
                /* erase the unsupported language in config files*/
                {
                    ...
                    if (cur_lang_info)
                    {
                        cur_lang = static_cast<wxLanguage>(cur_lang_info->Language);
                    }
            ...
            if (!wxLocale::IsAvailable(language_info->Language) && m_language_info_system)
            {
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format("Can't switch language to %1% (missing locales). Using %2% instead.") % original_lang % language_info->CanonicalName.ToUTF8().data();
            }
#endif
        ...
        ConfigOptionMode GUI_App::get_mode()
        {
            ...
            return mode == "advanced" ? comAdvanced : mode == "simple" ? comSimple
                                                  : mode == "develop"  ? comDevelop
                                                                       : comSimple;
        }
        ...
        {
            // QDS
            const std::string mode_str = mode == comAdvanced ? "advanced" : mode == comSimple ? "simple"
        ...
        int GUI_App::extruders_cnt() const
        {
            ...
            return preset.printer_technology() == ptSLA ? 1 : preset.config.option<ConfigOptionFloatsNullable>("nozzle_diameter")->values.size();
        }
        ...
        bool GUI_App::run_wizard(ConfigWizard::RunReason reason, ConfigWizard::StartPage start_page)
        {
            ...
            auto page = start_page == ConfigWizard::SP_WELCOME ? GuideFrame::QDT_WELCOME : start_page == ConfigWizard::SP_FILAMENTS ? GuideFrame::QDT_FILAMENT_ONLY
        ...
        void TryLoadLastMachine::InnerLoad(NetworkAgent *agent, DeviceManager *dev)
        {
            if (is_mqtt_ok && is_list_ok)
            {
                if ((dev->get_selected_machine() == nullptr) && (dev->get_user_machinelist().size() > 0))
                    dev->set_selected_machine(agent->get_user_selected_machine());
            }
        }
