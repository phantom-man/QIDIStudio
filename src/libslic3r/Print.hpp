...
#define TIME_GENERATE_SUPPORT "generate_support_material_time"
    ...
    public:
        ...
        struct LayerRangeRegions
        {
            ...
            {
                auto it = lower_bound_by_predicate(this->volumes.begin(), this->volumes.end(), [id](const VolumeExtents &l)
    ...
    */
    ...
    public:
        ...
        {
            return std::any_of(m_objects.begin(), m_objects.end(), [](PrintObject *object)
                               { return object->config().brim_type == btAutoBrim; });
        }
        ...
        const PrintObject *get_object(ObjectID object_id) const
        {
            auto it = std::find_if(m_objects.begin(), m_objects.end(),
                                   [object_id](const PrintObject *obj)
        ...
        {
            return std::all_of(this->objects().begin(), this->objects().end(), [&](PrintObject *obj)
                               { return obj->height() < scale_(this->config().nozzle_height.value); });
        }
