...
namespace Slic3r
{
                         ...
                         size_t bytes_transferred)
    {
        ...
        if (!line.empty())
        {
            ...
            // parse incoming line
            if (!this->connected && (boost::starts_with(line, "start") || boost::starts_with(line, "Grbl ") || boost::starts_with(line, "ok") || boost::contains(line, "T:")))
            {
                this->connected = true;
                {
                    boost::lock_guard<boost::mutex> l(this->queue_mutex);
    ...
    GCodeSender::do_send()
    {
        ...
        os << full_line;
        boost::asio::async_write(this->serial, this->write_buffer, boost::bind(&GCodeSender::on_write, this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
    }
