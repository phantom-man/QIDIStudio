...
namespace Slic3r
{
    ...
    public:
        ...
        {
            return this->first_compatible_idx([](const Preset &) -> int
                                              { return 0; });
        }
        ...
        {
            this->update_compatible(active_printer, active_print, select_other_if_incompatible, [](const Preset &) -> int
                                    { return 0; });
        }

        size_t num_visible() const
        {
            return std::count_if(m_presets.begin(), m_presets.end(), [](const Preset &preset)
    ...
    private:
        ...
        {
            auto it = Slic3r::lower_bound_by_predicate(m_presets.begin() + m_num_default_presets, m_presets.end(), [&name](const auto &l)
                                                       { return l.name < name; });
    ...
    {
    public:
        PrinterPresetCollection(Preset::Type type, const std::vector<std::string> &keys, const Slic3r::StaticPrintConfig &defaults, const std::string &default_name = "Default Printer") : PresetCollection(type, keys, defaults, default_name) {}
