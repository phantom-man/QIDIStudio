...
namespace Slic3r
{
    ...
    {
        return find_closest_points<K>(kdtree, point, [](size_t)
                                      { return true; });
    }
    ...
    {
        return find_closest_point(kdtree, point, [](size_t)
                                  { return true; });
    }
                                           ...
                                           const typename KDTreeIndirectType::CoordType &max_distance, FilterFn filter)
    {
        ...
        struct Visitor
        {
            ...
            Visitor(const KDTreeIndirectType &kdtree, const PointType &center, const CoordType &max_distance,
                    FilterFn filter) : kdtree(kdtree), center(center), max_distance_squared(max_distance * max_distance), filter(filter)
            {
            }
            unsigned int operator()(size_t idx, size_t dimension)
            {
                                           ...
                                           FilterFn filter)
    {
        struct Visitor
        {
            ...
            Visitor(const KDTreeIndirectType &kdtree, const PointType &bbmin, const PointType &bbmax,
                    FilterFn filter) : kdtree(kdtree), bb_min{bbmin}, bb_max{bbmax}, filter(filter)
            {
            }
            unsigned int operator()(size_t idx, size_t dimension)
            {
