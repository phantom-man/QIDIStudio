...
namespace Slic3r
{
    ...
    inline ExPolygons remove_duplicates(ExPolygons expolygons, coord_t scaled_eps, double max_angle)
    {
        ...
        for (ExPolygon &expolygon : expolygons)
        {
            ...
            expolygon.holes.erase(std::remove_if(expolygon.holes.begin(), expolygon.holes.end(), [](const auto &p)
                                                 { return p.empty(); }),
                                  expolygon.holes.end());
        }
        expolygons.erase(std::remove_if(expolygons.begin(), expolygons.end(), [](const auto &p)
                                        { return p.empty(); }),
                         expolygons.end());
        return expolygons;
    }
    ...
    inline Polygons smooth_outward(Polygons polygons, coord_t clip_dist_scaled)
    {
        ...
        polygons.erase(std::remove_if(polygons.begin(), polygons.end(), [](const auto &p)
                                      { return p.empty(); }),
                       polygons.end());
        return polygons;
    }
