...
namespace Slic3r
{
    ...
    namespace Geometry
    {
        ...
        template <typename T>
        inline T foot_pt(const T &line_pt, const T &line_dir, const T &pt)
        {
            T v = pt - line_pt;
        ...
        template <typename T>
        inline auto ray_point_distance_squared(const T &ray_pt, const T &ray_dir, const T &pt)
        {
            return (foot_pt(ray_pt, ray_dir, pt) - pt).squaredNorm();
        }

        template <typename T>
        inline auto ray_point_distance(const T &ray_pt, const T &ray_dir, const T &pt)
        {
        ...
        template <typename T>
        T rad2deg(T angle) { return T(180.0) * angle / T(PI); }
        double rad2deg_dir(double angle);
        template <typename T>
        constexpr T deg2rad(const T angle) { return T(PI) * angle / T(180.0); }
        template <typename T>
        ...
        public:
            ...
            {
                m_temp_offset = m_matrix.translation();
                return m_temp_offset;
            }
        ...
        private:
            ...
            void serialize(Archive &ar) { ar(m_matrix); }
