...
namespace Slic3r
{
    ...
    {
    public:
        file_parser_error(const std::string &msg, const std::string &file, unsigned long line = 0) : Slic3r::RuntimeError(format_what(msg, file, line)),
                                                                                                     m_message(msg), m_filename(file), m_line(line) {}
        file_parser_error(const std::string &msg, const boost::filesystem::path &file, unsigned long line = 0) : Slic3r::RuntimeError(format_what(msg, file.string(), line)),
