...
#define OBJECTBASE_DERIVED_COPY_MOVE_CLONE(TYPE)                                           \
    ...
    struct CutConnectorAttributes
    {
        ...
        template <class Archive>
        inline void serialize(Archive &ar) { ar(type, style, shape); }
    };
    ...
    struct CutConnector
    {
        ...
        template <class Archive>
        inline void serialize(Archive &ar) { ar(pos, rotation_m, radius, height, radius_tolerance, height_tolerance, attribs); }
    };
    ...
    public:
        ...
        template <typename T>
        const T *get_config_value(const DynamicPrintConfig &global_config, const std::string &config_option)
        {
    ...
    struct RaycastResult
    {
        ...
        void serialize(Archive &ar) { ar(mouse_position, mesh_id, hit, normal); }
    };
    ...
    public:
        ...
        struct CutInfo
        {
            ...
            CutInfo(CutConnectorType type, float radius_, float height_, float rad_tolerance, float h_tolerance, bool processed = false)
                : is_connector(true), is_processed(processed), connector_type(type), radius(radius_), height(height_), radius_tolerance(rad_tolerance), height_tolerance(h_tolerance)
            {
            }
            ...
            template <class Archive>
            inline void serialize(Archive &ar) { ar(is_connector, is_processed, connector_type, radius_tolerance, height_tolerance); }
        };
        CutInfo cut_info;
    ...
    private:
        ...
        ModelVolume(ModelObject *object, const ModelVolume &other) : ObjectBase(other),
                                                                     name(other.name), source(other.source), m_mesh(other.m_mesh), m_convex_hull(other.m_convex_hull),
                                                                     config(other.config), m_type(other.m_type), object(object), m_transformation(other.m_transformation), supported_facets(other.supported_facets), fuzzy_skin_facets(other.fuzzy_skin_facets), seam_facets(other.seam_facets), mmu_segmentation_facets(other.mmu_segmentation_facets), m_text_info(other.m_text_info), emboss_shape(other.emboss_shape)
        {
            assert(this->id().valid());
            assert(this->config.id().valid());
            assert(this->supported_facets.id().valid());
        ...
        ModelVolume(ModelObject *object, const ModelVolume &other, const TriangleMesh &&mesh) : name(other.name), source(other.source), m_mesh(new TriangleMesh(std::move(mesh))), config(other.config), m_type(other.m_type), object(object), m_transformation(other.m_transformation),
    ...
    {
        std::sort(model_volumes.begin(), model_volumes.end(), [](const ModelVolume *l, const ModelVolume *r)
                  { return l->id() < r->id(); });
    }

    inline const ModelVolume *model_volume_find_by_id(const ModelVolumePtrs &model_volumes, const ObjectID id)
    {
        auto it = lower_bound_by_predicate(model_volumes.begin(), model_volumes.end(), [id](const ModelVolume *mv)
                                           { return mv->id() < id; });
    ...
    private:
        ...
        // Constructor, which assigns a new unique ID.
        explicit ModelInstance(ModelObject *object, const ModelInstance &other) : m_transformation(other.m_transformation), m_assemble_transformation(other.m_assemble_transformation), m_offset_to_assembly(other.m_offset_to_assembly), print_volume_state(ModelInstancePVS_Inside), printable(other.printable), object(object), m_assemble_initialized(false) { assert(this->id().valid() && this->id() != other.id()); }
    ...
    private:
        ...
        template <typename Archive>
        void serialize(Archive &ar) { ar(positions, rotation); }
    };
...
namespace cereal
{
    ...
    // QDS: backup
    template <class Archive>
    struct specialize<Archive, Slic3r::Model, cereal::specialization::member_load_save>
    {
    };
    template <class Archive>
    struct specialize<Archive, Slic3r::ModelObject, cereal::specialization::member_load_save>
    {
    };
    template <class Archive>
