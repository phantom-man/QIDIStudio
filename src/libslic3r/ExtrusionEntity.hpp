...
namespace Slic3r
{
    ...
    {
        return role == erPerimeter || role == erExternalPerimeter || role == erOverhangPerimeter;
    }

    inline bool is_infill(ExtrusionRole role)
    {
        return role == erBridgeInfill || role == erInternalInfill || role == erSolidInfill || role == erFloatingVerticalShell || role == erTopSolidInfill || role == erBottomSurface || role == erIroning;
    }
    ...
    {
        return role == erBridgeInfill || role == erSolidInfill || role == erFloatingVerticalShell || role == erTopSolidInfill || role == erBottomSurface || role == erIroning;
    }

    inline bool is_bridge(ExtrusionRole role)
    {
        return role == erBridgeInfill || role == erOverhangPerimeter;
    }
    ...
    public:
        ...
        ExtrusionPath(const ExtrusionPath &rhs)
            : ExtrusionEntity(rhs), polyline(rhs.polyline), overhang_degree(rhs.overhang_degree), curve_degree(rhs.curve_degree), mm3_per_mm(rhs.mm3_per_mm), width(rhs.width), height(rhs.height), smooth_speed(rhs.smooth_speed), m_can_reverse(rhs.m_can_reverse), m_role(rhs.m_role), m_no_extrusion(rhs.m_no_extrusion)
        {
        }
        ExtrusionPath(ExtrusionPath &&rhs)
            : ExtrusionEntity(rhs), polyline(std::move(rhs.polyline)), overhang_degree(rhs.overhang_degree), curve_degree(rhs.curve_degree), mm3_per_mm(rhs.mm3_per_mm), width(rhs.width), height(rhs.height), smooth_speed(rhs.smooth_speed), m_can_reverse(rhs.m_can_reverse), m_role(rhs.m_role), m_no_extrusion(rhs.m_no_extrusion)
        {
        }
        ExtrusionPath(const Polyline &polyline, const ExtrusionPath &rhs)
            : ExtrusionEntity(rhs), polyline(polyline), overhang_degree(rhs.overhang_degree), curve_degree(rhs.curve_degree), mm3_per_mm(rhs.mm3_per_mm), width(rhs.width), height(rhs.height), smooth_speed(rhs.smooth_speed), m_can_reverse(rhs.m_can_reverse), m_role(rhs.m_role), m_no_extrusion(rhs.m_no_extrusion)
        {
        }
        ExtrusionPath(Polyline &&polyline, const ExtrusionPath &rhs)
            : ExtrusionEntity(rhs), polyline(std::move(polyline)), overhang_degree(rhs.overhang_degree), curve_degree(rhs.curve_degree), mm3_per_mm(rhs.mm3_per_mm), width(rhs.width), height(rhs.height), smooth_speed(rhs.smooth_speed), m_can_reverse(rhs.m_can_reverse), m_role(rhs.m_role), m_no_extrusion(rhs.m_no_extrusion)
        {
        }
